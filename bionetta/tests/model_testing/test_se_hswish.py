...
class SETest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        x = SEHeavyBlock(kernel_size=4, activation=HardSwish())(inputs)
        x = tf.keras.layers.Flatten(input_shape=(28,28,1))(x)
        x = tf.keras.layers.Dense(units = 64)(x)
        x = HardSwish()(x)  # LeakyReLU with alpha=1/(2**5)=1/32
    ...
    def test_SE(self) -> None:
        test_model(self, './compiled_se_hswish', proving_backend=ProvingBackend.GROTH16())