...
class EdTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        x = EDLight2DConv(kernel_size=4, kernel_output_size=4, channels=3, activation=activation)(inputs)
        x = tf.keras.layers.Flatten(input_shape=(28,28,3))(x)
        x = tf.keras.layers.Dense(units = 64)(x)
        x = activation(x) # LeakyReLU with alpha=1/(2**5)=1/32
    ...
    def test_ED(self) -> None:
        test_model(self, './compiled_ed', self.proving_backend)
